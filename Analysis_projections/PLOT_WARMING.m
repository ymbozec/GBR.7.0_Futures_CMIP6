...
for ssp = 1:5
    ...
    %% USING THE APPLIED DHW
    % load('DHW_scenarios.mat') % Contains scenarios 20x3806x93 of DHW for each ssp and gcm [all_DHW_scenarios(ssp,gcm)]
    % load('/home/ym/REEFMOD/REEFMOD.6.8_GBR/data/GBR_REEF_POLYGONS_2023.mat')
    %
    ...
    %     Stress_levels ={'Low stress','Moderate','Strong','Severe', 'Extreme'};
    % DHW_edges = [0 3 6 9 12 100];
    
%     Stress_levels ={'Low stress', 'Moderate','Severe','Extreme'};
    %     DHW_edges = [0 3 6 12 100];
%     DHW_edges = [0 4 8 12 100];
%     DHW_edges = [0 8 16 24 100];
%     Stress_levels ={'0-8ºC-week','8-16ºC-week','16-24ºC-week','>24ºC-week'};
    ...
    %% Other way: proportion of reef for each simulation then average the counts every year
    % Edit: does the same
    % Y = nan(100,93,length(Stress_levels));
    %
    % for simul=1:100
    %     for yr = 1:93
    %
    %         [counts, edges] = histcounts(ALL_DHWs(simul,:,yr),DHW_edges);
    %         Y(simul, yr,:) = counts/sum(counts);
    %     end
    % end
    % Z = squeeze(mean(Y,1));
